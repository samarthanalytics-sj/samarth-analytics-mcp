...
function buildRecipe(a: { accountId?: string; containerName?: string; measurementId?: string; adsConversionId?: string }): string {
  ...
  return [
    ...
    '',
    '3. CREATE THE CLIENTS — clients_create; pass each resource as a JSON STRING in the bodyJson argument (clients_create takes bodyJson, not structured fields).',
    '   GA4 CLIENT (claims incoming GA4/gtag requests; the cookie parameters enable SERVER-MANAGED first-party FPID cookies — the httpOnly identifier that survives ITP; production pattern, corpus-validated):',
    '   bodyJson: \'{ "name":"GA4", "type":"gaaw_client", "parameter":[ {"type":"boolean","key":"activateDefaultPaths","value":"true"}, {"type":"boolean","key":"activateGtagSupport","value":"true"}, {"type":"template","key":"cookieManagement","value":"server"}, {"type":"template","key":"cookieName","value":"FPID"}, {"type":"template","key":"cookieDomain","value":"auto"}, {"type":"template","key":"cookiePath","value":"/"}, {"type":"template","key":"cookieMaxAgeInSec","value":"63072000"}, {"type":"boolean","key":"migrateFromJsClientId","value":"true"} ] }\'',
    '   GTM CLIENT (recommended when a WEB container exists — the tagging server FIRST-PARTY-SERVES gtm.js for it; use the web container\'s public GTM-XXXXXXX id):',
    '   bodyJson: \'{ "name":"GTM Web Container", "type":"gtm_client", "parameter":[ {"type":"boolean","key":"activateResponseCompression","value":"true"}, {"type":"boolean","key":"activateGeoResolution","value":"false"}, {"type":"boolean","key":"activateDependencyServing","value":"true"}, {"type":"list","key":"allowedContainerIds","list":[ {"type":"map","map":[ {"type":"template","key":"containerId","value":"GTM-XXXXXXX"} ]} ]} ] }\'',
    '',
    ...
    '   EVENT NAMES MUST BE EXACT — type them as they arrive, with spaces ("Sign Petition Click"), never URL-encoded ("Sign+Petition+Click"): a pasted-encoded value silently never matches and the tag never fires. To ALSO scope a trigger to one page/campaign, add a second filter {"type":"contains","parameter":[ {"type":"template","key":"arg0","value":"{{ed - page_location}}"}, {"type":"template","key":"arg1","value":"/petition/…/"} ]} (create the ed - page_location Event Data variable first, keyPath "page_location").',
    '',
    '6. CREATE TAGS (tags_create — structured args: name, type, parameter, firingTriggerId) — each with firingTriggerId = ["<triggerId>"] (an ARRAY of trigger ids, usually one):',
    ...
    '',
    '7. WIRE THE HOST (after the user deploys the tagging server) — record the URL on the SERVER container with containers_set_tagging_server_urls (serverUrls: ["https://…"]), then point the web container at it: update the WEB container\'s Google tag (tags_update) adding {"type":"map","map":[{"type":"template","key":"parameter","value":"server_container_url"},{"type":"template","key":"parameterValue","value":"https://…"}]} to its configSettingsTable list parameter. Confirm the host answers before relying on it.',
    '',
