...
const SERVER_MIGRATION_HEURISTICS: ReadonlyArray<{
  ...
  { re: /plausible/i, destination: 'Plausible', serverTool: 'templates_import_from_gallery (mbaersch/plausible-analytics-tag-server) + tags_create', requires: [], status: 'generic',
  ...
  { re: /umami/i, destination: 'Umami', serverTool: 'templates_import_from_gallery (mbaersch/umami-tag-server) + tags_create', requires: [], status: 'generic',
  ...
  { re: /pirsch/i, destination: 'Pirsch', serverTool: 'templates_import_from_gallery (mbaersch/pirsch-tag-server) + tags_create', requires: ['token'], status: 'generic',
    note: 'Pirsch server tag; fields endpointUrl (api.pirsch.io/api/v1 by default), token (a Pirsch ACCESS token - not the web identification code), setUrlVar/setEventVar. Published by mbaersch; stape-io only mirrors it.' },
  { re: /snowplow|newTracker\s*\(/i, destination: 'Snowplow', serverTool: 'templates_import_from_gallery (snowplow/snowplow-gtm-server-side-tag) + tags_create', requires: [], status: 'generic',
  ...
  { re: /tapfiliate|\btap\s*\(\s*['"]create['"]/i, destination: 'Tapfiliate', serverTool: 'MANUAL template install (stape-io/tapfiliate-tag) + tags_create', requires: ['apiKey'], status: 'generic',
    note: 'Tapfiliate server tag; fields apiKey (the web tap(\'create\', ...) account id is NOT it), referralCodeKey (ref), customerId/customerStatus. NOT in the GTM gallery: upload stape-io/tapfiliate-tag template.tpl into the server container by hand first.' },
