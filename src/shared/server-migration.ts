...
const SERVER_MIGRATION_HEURISTICS: ReadonlyArray<{
  ...
  { re: /tapfiliate|\btap\s*\(\s*['"]create['"]/i, destination: 'Tapfiliate', serverTool: 'templates_import_from_gallery (stape-io/tapfiliate-tag) + tags_create', requires: ['apiKey'], status: 'generic',
    note: 'Tapfiliate server tag; fields apiKey (the web tap(\'create\', ...) account id is NOT it), referralCodeKey (ref), customerId/customerStatus. NOT in the GTM gallery, so templates_import_from_gallery installs it from the vendor source instead of importing it.' },
