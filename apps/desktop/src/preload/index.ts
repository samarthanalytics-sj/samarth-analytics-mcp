...
const api = {
  ...
  // Open a URL in the user's real Chrome specifically (verify "Open in my Chrome" button, so a pasted Tag
  ...
  tags: {
    ...
    // proof images), 'pdf', or 'docx' (a real Word doc with each proof embedded as an image; uploads to
    // Google Docs intact). Returns path or null.
    exportVerifyResults: (format: 'xlsx' | 'pdf' | 'docx', defaultName: string, payload: VerifyExportPayload): Promise<string | null> =>
