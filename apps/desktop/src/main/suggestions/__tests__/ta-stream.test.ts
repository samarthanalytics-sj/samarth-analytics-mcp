...
// ── full capture: the user's screenshot scenario ──────────────────────────────────
{
  const frames: unknown[] = [
    ...
    memo('G-EXAMPLE22', 16, 'form_submission', 'TAG_STATUS', { tagInfo: [{ name: '_Tagging Activity Tag 4', execute: 'execute_running' }] }),
  ];
  ...
  check('isolation: gtag container has its own event', eventsForContainer(cap, 'G-EXAMPLE22').length === 1);
}
