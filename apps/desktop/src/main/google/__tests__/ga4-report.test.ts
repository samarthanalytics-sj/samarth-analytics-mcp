...
test('area-status table reflects the config areas (with confidence)', () => {
  const md = buildGa4AuditReport(input());
  assert.ok(md.includes('| Data collection | Pass | Certain |'));
  assert.ok(/\| Custom definitions \| Pass \| Certain \|/.test(md));
});
