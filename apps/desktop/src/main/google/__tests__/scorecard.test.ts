...
test('severity weighting (high 15 / med 7 / low 3 / info 0) and floor at 0', () => {
  // 1 high + 1 medium + 1 low = 100 - 25 = 75
  assert.equal(buildScorecard([{ key: 'g', label: 'G', findings: [f('high'), f('medium'), f('low')] }]).score, 75);
  // info never deducts
...
test('overall score pools all sections; per-section scores are independent', () => {
  ...
  assert.equal(ga4.score, 70);
  assert.equal(ga4.grade, 'C');
  assert.equal(r.score, 70, 'overall = combined pool (100 - 30)');
