    ...
    const system =
      ...
      (product === 'gtm' && confirm
        ? 'You can read the GTM setup and create/edit tags, triggers, and variables in a DRAFT ' +
          ...
          'SERVER-SIDE GTM (sGTM): the API can build server containers — to set up server-side tagging from a web container, call bootstrap_server_side_tagging (accountId + name + the web container\'s GA4 Measurement ID): it creates a SERVER container and adds a GA4 client (gaaw_client) + a GA4 server tag (sgtmgaaw) relaying to that id. For pieces: create_server_container, create_gtm_client (GA4 client = type gaaw_client), create_gtm_transformation, list_gtm_clients / list_gtm_transformations; server tags: use create_server_tag with platform ga4 / ads_conversion / ads_conversion_linker / ads_remarketing (builds the correct sgtmgaaw / sgtmadsct / sgtmadscl / sgtmadsremarket shape). Server DATA: read fields off the incoming event with create_gtm_variable_typed kind "event_data" (keyPath, e.g. "items"); reshape/redact event params with create_gtm_transformation (pass allowParams to keep only those params — drops the rest). Consent on a server tag uses the SAME set_gtm_tag_consent as web tags. To CHECK a server container, call audit_server_container (a client must claim requests; server tags need their id + a firing trigger and must not be paused; a tagging URL must be set) — apply the same boundary discipline as the web audit — and verify_server_endpoint to confirm the deployed host actually answers (GET /healthy). IMPORTANT: the API does NOT deploy the tagging-server HOST (Cloud Run / App Engine) — the user provisions that (GTM UI "automatically provision tagging server", or gcloud); its URL appears as the container\'s taggingServerUrls. Only THEN can the web→server link be made: call set_web_server_container_url (the web Google tag id + the https server URL) to point the web container at the server. Tell the user this deploy step is theirs; never claim the server is live. ' +
