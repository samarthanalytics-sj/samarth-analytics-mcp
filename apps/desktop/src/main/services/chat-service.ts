...
export const GTM_AUDIT_METHODOLOGY =
  ...
  '(8) SEVERITY nuance: a paused tag is Low, BUT a paused conversion (Ads) or GA4/Google CONFIG tag is High — a silent tracking gap. A tag with an empty destination id (no Measurement/Tag/Conversion id) is High — it looks active but sends nothing. A tag whose Measurement/Tag id is a {{variable}} is NOT a defect — track it as runtime-required (verify it resolves to a real G-/AW-/GT- id on a live load). ' +
