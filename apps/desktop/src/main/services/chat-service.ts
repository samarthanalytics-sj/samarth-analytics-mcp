...
export const GTM_AUDIT_METHODOLOGY =
  ...
  '(7) CONSENT: Consent Mode v2 needs ALL FOUR signals (ad_storage, analytics_storage, ad_user_data, ad_personalization) with a denied-by-default Consent Initialization — flag missing signals as Critical, but consent TIMING/firing-order is runtime-required. CUSTOM HTML has NO built-in Consent Mode: an advertising pixel pasted as Custom HTML (Meta/Facebook, TikTok, LinkedIn, Pinterest, Snapchat, X/Twitter, Reddit, Microsoft/Bing UET) with consentStatus notSet fires UNGATED, drops ad cookies and sends user data regardless of consent — CRITICAL for EU/UK/AU (B6). Google tags (GA4/Ads/Floodlight/Linker) DO have built-in consent, so notSet on them is [Likely], not certain; never skip a non-Google marketing tag\'s missing consent gate as "nothing to check". ' +
  '(8) SEVERITY nuance: a paused tag is Low, BUT a paused conversion (Ads) or GA4/Google CONFIG tag is High — a silent tracking gap. A tag with an empty destination id (no Measurement/Tag/Conversion id) is High — it looks active but sends nothing. ' +
  '(9) END with an explicit list of the runtime-required checks not yet verified, so nobody assumes they passed. After applying fixes, re-run audit_gtm_container to confirm they cleared. ';
