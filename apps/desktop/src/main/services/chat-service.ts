...
export const GA4_TAG_NAMING =
  'GA4 TAG NAMING — unless the user gives an explicit name, name every GA4 event tag you create "GA4 - Event - <Name> Tag" and its Custom Event trigger "<Name> Trigger", where <Name> is the event in Title Case: add_to_cart → tag "GA4 - Event - Add To Cart Tag" + trigger "Add To Cart Trigger"; view_item → "GA4 - Event - View Item Tag" + "View Item Trigger"; purchase → "GA4 - Event - Purchase Tag" + "Purchase Trigger". Apply this format to ALL GA4 tags/triggers you create. CRITICAL — a Custom Event trigger has TWO different fields: its display NAME (e.g. "Purchase Trigger" / "CE - Purchase") and its EVENT NAME (the dataLayer value it matches). The EVENT NAME must be the raw event in snake_case exactly as the dataLayer pushes it (purchase, add_to_cart, view_item, begin_checkout, generate_lead, file_download) — NEVER a display label like "CE - Purchase" or "GA4 - Purchase" (the dataLayer never pushes that, so the trigger would never fire). Use snake_case underscore_words for the event name; reserve the "CE - "/"GA4 - " prefixes for the display name only. ';
    ...
    const system =
      ...
      (product === 'gtm' && confirm
        ? 'You can read the GTM setup and create/edit tags, triggers, and variables in a DRAFT ' +
          ...
          'TIKTOK EVENTS API (server-side): to forward events to TikTok server-side, use create_tiktok_capi_server_tag (Stape stape-io/tiktok-tag, match-quality-tuned: Event Enhancement + generate _ttp ON). Pass pixelId + accessToken (the TikTok Events Manager access token — usually {{variables}}) + event. This is the SERVER Events API tag and is DISTINCT from the TikTok WEB pixel (tiktok/gtm-template-pixel) — it uses different field keys (pixelId/accessToken/eventName, NOT the web pixel_code/event), so never hand-build it with the web keys. Standard TikTok events: CompletePayment(=purchase), AddToCart, ViewContent, InitiateCheckout, Lead, CompleteRegistration, SubmitForm, Search, AddToWishlist, AddPaymentInfo, PlaceAnOrder, Download, Subscribe, Contact, ClickButton; GA4 names are mapped automatically (purchase→CompletePayment, add_to_cart→AddToCart, view_item→ViewContent, begin_checkout→InitiateCheckout, generate_lead→Lead); an unrecognised event becomes a TikTok custom event. MATCH QUALITY (TikTok\'s EMQ analogue): pass userData rows for advanced matching — name one of email/phone/external_id/ttclid/ttp/ip/user_agent/first_name/last_name/city/state/country/zip_code, values usually {{variables}} (email + phone + ttclid are highest-value; ttclid/ttp/ip/user_agent auto-collect from the incoming event when omitted). For deduplication with the web TikTok pixel, send the SAME eventId on both. eventProperties carry the commerce data (currency/value/contents/content_ids/content_type/num_items/order_id). Like Meta CAPI, this tag needs a SERVER trigger — create_server_trigger scoped to the client that claims the events — and pass its firingTriggerId. ' +
