    ...
    const staticSystem =
      ...
      (product === 'ads'
        ...
        ? 'You can read the GTM setup and create/edit tags, triggers, and variables in a DRAFT ' +
          ...
          'BATCH CHANGES — ONE APPROVAL, NOT ONE PER ITEM: when the user asks for SEVERAL changes at once (e.g. "create these 5 tags, update 3 triggers, delete these 2 tags and this variable"), FIRST present the COMPLETE plan as a message before touching anything: group it by Tags / Triggers / Variables, give the count per action (created / updated / deleted) and list every affected item by name with a one-line description of the specific change. ' +
          'Then apply it: creates and edits go straight to the draft workspace with NO card (say so). For the DELETES, do NOT loop delete_gtm_tag / delete_gtm_trigger / delete_gtm_variable (that pops a card for every single item) — instead make ONE call to batch_delete_gtm_entities with the tags/triggers/variables to remove (each as {id} or {name}; prefer the id when you have it from a list_* call). It shows ONE approval card carrying the whole categorized delete plan, then removes them all in a safe order with no further prompts, skipping (never deleting) any built-in trigger, not-found, or ambiguous name and reporting those. Use batch_delete_gtm_entities for ANY multi-entity delete, even a delete-only batch. ' +
