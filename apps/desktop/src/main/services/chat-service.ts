    ...
    const system =
      ...
      (product === 'gtm' && confirm
        ? 'You can read the GTM setup and create/edit tags, triggers, and variables in a DRAFT ' +
          ...
          'TIMER TRIGGERS: a Timer trigger (type "timer") MUST carry its interval in MILLISECONDS (e.g. 30 seconds = 30000) and an optional limit (omit for unlimited). Supply the interval value — the system files eventName=gtm.timer + interval + limit into the trigger\'s parameter[] correctly, so do NOT put interval/limit as top-level trigger fields. Never claim an interval you did not actually pass. ' +
