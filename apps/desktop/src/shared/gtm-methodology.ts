...
export const GTM_TRIGGER_VARIABLE_REFERENCE =
  ...
  'WHICH TOOL: TYPED builders cover the common cases and are PREFERRED — create_gtm_tracking_tag (tag + trigger + built-ins in one; trigger.kind = pageview | all_clicks | link_click | form_submit | custom_event | youtube_video) and create_gtm_variable_typed (kind = constant | data_layer | javascript [= Custom JavaScript] | event_data [server only]). Every OTHER trigger/variable type has NO typed builder — create it with the RAW tools create_gtm_trigger / create_gtm_variable, passing a correctly shaped GTM API v2 resource (shapes below); never guess the shape, and if a create is rejected the field keys are wrong for that type, so fix them. ' +
  ...
  'These need RAW create_gtm_trigger: DOM Ready [type domReady] (tag must read the DOM); Window Loaded [windowLoaded] (needs late / 3rd-party content); Element Visibility [elementVisibility] (an element enters the viewport — section / sticky CTA / banner seen); History Change [historyChange] (SPA route change — pushState/hash, i.e. virtual pageviews); Scroll Depth [scrollDepth] (25/50/75/90%); Timer [timer] (time-on-page; interval ms + limit are TOP-LEVEL fields); JavaScript Error [jsError]; Trigger Group [triggerGroup] (fire only after ALL listed triggers fired). Filter operators: equals | contains | startsWith | endsWith | matchRegex | greater | less. Attach a trigger under a tag\'s EXCEPTIONS to BLOCK it (internal traffic, consent denied). ' +
  'VARIABLES — when to use each: Data Layer Variable (create_gtm_variable_typed kind data_layer; read a dataLayer key, dot-notation for nested e.g. ecommerce.value — the primary source of developer-pushed values). Constant (kind constant; a fixed reused id — GA4 Measurement ID, conversion label). Custom JavaScript (kind javascript; a function(){…} that computes/normalises a value and can read other {{vars}} — e.g. trim + lowercase a label). Event Data (kind event_data; server container only). ' +
