...
export const GTM_DECISION_RULES =
  ...
  'CLICK — match on the most STABLE field, in order: a stable id ({{Click ID}}) or a data-* attribute ({{Click Element}} matches CSS selector [data-cta="…"]) FIRST; then {{Click URL}} for links; {{Click Text}} / {{Click Classes}} LAST (visible text changes across languages/redesigns, framework class names churn — both break quietly). For links use Click - Just Links + turn ON "Wait for Tags" so the tag fires before navigation; for buttons use Click - All Elements but ALWAYS add a condition (never fire on every click). ' +
