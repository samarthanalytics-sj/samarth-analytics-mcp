...
// ── the connected-platform write allowlist (a chip grants a workflow, not an admin surface) ──
{
  ...
  check('allowlist: Ads contributes ONLY the conversion-action creates (single + batch)', CONNECTED_WRITE_ALLOWLIST.ads.length === 2 && connectedWriteAllowed('ads', 'create_google_ads_conversion_action') && connectedWriteAllowed('ads', 'create_google_ads_conversion_actions_for_tags'));
