...
export const AUDIT_REPORTING_METHODOLOGY =
  ...
  '(1b) INVENTORY FIRST. The result carries an `inventory` object ({tags, triggers, variables}). Open with one line naming the container and workspace, then render inventory as THREE GitHub-flavoured Markdown tables, reproducing EVERY row exactly and never summarising, sampling, or dropping rows: ' +
  'TAG AUDIT with columns "Tag Name | Type | Firing Triggers | Firing Option" (inventory.tags: name / type / firingTriggers / firingOption); ' +
  'TRIGGER AUDIT with columns "Trigger Name | Type | Firing Conditions / Filters | Usage Status" (inventory.triggers: name / type / conditions / usage); ' +
  'VARIABLE AUDIT with columns "Variable Name | Type | Value / Configuration" (inventory.variables: name / type / value). ' +
  'Render ONLY values present in `inventory` — never invent a condition, firing option, type label, or usage status, and never fill a blank value cell with a guess. After the three tables, give the severity summary and score, then the findings. ' +
  ...
  '(11) END the findings with that explicit runtime-required list so nobody assumes those checks passed. ' +
  '(12) CLEANING ACTION ITEMS & RECOMMENDATIONS: after the findings, add a section under that exact heading that turns the actionable findings into plain-language cleanup steps (e.g. "Delete the 2 unused triggers: X, Y"; "Form tags set to Once per load — a re-submit after a validation error may not re-fire; switch to Once per event"). Then ask, in one line, which actions to proceed with, and CHANGE NOTHING until the user replies. When they confirm, APPLY the changes by calling each finding\'s ready-to-run `fix` tool (non-delete fixes apply directly to the draft workspace; deletes show the two-step approval card) — never tell the user to make the change manually in the GTM UI when a fix tool exists. After applying, re-run audit_gtm_container to confirm they cleared. ';
