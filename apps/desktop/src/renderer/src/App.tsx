  ...
  return (
    <div style={styles.page}>
      <header style={styles.header}>
        ...
        <span style={styles.badge}>Phase 1 — accounts + secrets</span>
      </header>
      <p style={styles.lede}>
        Local multi-account desktop app for the GTM / GA4 MCP. Account registry and the
        encrypted secret store (Windows DPAPI) are live.
      </p>
      ...
      <section style={styles.card}>
        <div style={styles.cardHead}>
          ...
          <span style={styles.muted}>{accounts.length} stored</span>
        </div>
        ...
        <div style={styles.addRow}>
          ...
          <button
            style={styles.button}
            onClick={() => run(async () => {
              await window.desktop.accounts.add({ email });
              setEmail('');
            })}
          >
            Add account
        ...
        <p style={styles.hint}>
          Phase 1 stores the account record only — real Google sign-in (and the vaulted
          token) arrives in Phase 2.
        </p>
            ...
            {accounts.map((a) => (
              <li key={a.id} style={styles.account}>
                ...
                <div style={styles.accountMeta}>
                  <span>google token: {a.hasGoogleToken ? '✓ vaulted' : '— none'}</span>
                  <span>
      ...
      <section style={styles.card}>
        ...
        {selfTest && (
          <p style={{ color: selfTest.ok ? '#34d399' : '#fca5a5' }}>
            {selfTest.ok ? '✓ ' : '✗ '}
            {selfTest.detail} (encryption available: {String(selfTest.encryptionAvailable)})
          </p>
        )}
        <button style={styles.button} onClick={() => window.desktop.secrets.selfTest().then(setSelfTest)}>
          Re-run encryption self-test
        </button>
      </section>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  button: { background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 14px', fontSize: 13, cursor: 'pointer' },
  ...
  hint: { color: '#6b7280', fontSize: 12, margin: '8px 0 4px' },
  ...
  error: { color: '#fca5a5', whiteSpace: 'pre-wrap', background: '#1f1416', border: '1px solid #7f1d1d', borderRadius: 8, padding: 10, maxWidth: 760 },
  dl: { margin: 0 },
