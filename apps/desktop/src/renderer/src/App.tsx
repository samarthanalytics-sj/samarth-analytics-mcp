...
function VerifyPanel({
  ...
  const [vExporting, setVExporting] = useState<false | 'xlsx' | 'pdf' | 'docx'>(false);
  ...
  async function downloadVerify(format: 'xlsx' | 'pdf' | 'docx'): Promise<void> {
                ...
                {(['xlsx', 'pdf', 'docx'] as const).map((fmt) => (
                  <button
                    ...
                    title={
                      ...
                      : fmt === 'docx' ? 'Word .docx with each proof embedded as a real image — opens in Word and uploads to Google Docs with the screenshots intact'
