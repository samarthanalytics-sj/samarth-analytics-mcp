...
t('decodeCfEmail round-trips a real payload', () => {
  for (const addr of ['sales@dealer-one.example', 'info@dealer-two.example', 'a@b.co']) {
...
t('looksLikeAddress rejects a heading that merely sits in a location block', () => {
  assert.equal(looksLikeAddress('1 Sample Street, Exampleville NSW 2000, Australia'), true);
...
t('a Cloudflare-obfuscated email link classifies as email with a real mailto', () => {
  const hex = cfEncode('sales@dealer-one.example', 0x3d);
  const el = classifyElement(raw({ tag: 'a', href: `https://dealer-site.example/cdn-cgi/l/email-protection#${hex}`, text: '[email protected]', className: 'wpgb-block-4 dealer-email' }), 'dealer-site.example');
  assert.equal(el?.kind, 'email');
  assert.equal(el?.href, 'mailto:sales@dealer-one.example');
});

t('the same link with the data-cfemail attribute also decodes', () => {
  const hex = cfEncode('info@dealer-two.example', 0x11);
  const el = classifyElement(raw({ tag: 'a', href: '/cdn-cgi/l/email-protection', cfEmail: hex, className: 'dealer-email' }), 'dealer-site.example');
  assert.equal(el?.kind, 'email');
  assert.equal(el?.href, 'mailto:info@dealer-two.example');
});

t('a Google Maps link is an ADDRESS click, not a generic outbound', () => {
  const el = classifyElement(raw({
    tag: 'a',
    href: 'https://www.google.com/maps/search/?api=1&query=1%20Sample%20Street%2C%20Exampleville%20NSW%202000',
    text: '1 Sample Street, Exampleville NSW 2000, Australia',
...
t('an ordinary third-party link is still outbound', () => {
  const el = classifyElement(raw({ tag: 'a', href: 'https://www.other-dealer.example/', text: 'other-dealer.example', className: 'dealer-website' }), 'dealer-site.example');
