 ...
 * /api/gtm/accounts
 *
 * Fully self-contained Vercel serverless handler. No imports outside of
 * `node:*` — mirrors /api/oauth/status which is known to be Vercel-safe.
 * The previous version imported from `api/_lib/portal-session`, and Vercel
 * was crashing with FUNCTION_INVOCATION_FAILED at module-evaluation time.
 * Everything below is inlined so cold-start cannot fail.
 *
 * Unauthenticated probes (no cookie / bad cookie) return a clean JSON
 * `401 {"error":"not_connected"}` before any network calls.
 */
  ...
  try {
    ...
    if (secret.length < 16) {
      return sendJson(res, 500, {
        error: "config_error",
        message:
          "PORTAL_SESSION_SECRET must be set on Vercel to a 32+ character random value.",
      });
    }

    const client = resolveOAuthClient();
    if (!client) {
      return sendJson(res, 503, { error: "oauth_not_configured" });
    }
    ...
    try {
      const data = await gtmFetch<{ account?: GtmAccount[] }>(
        token,
        "/accounts",
      );
    ...
    console.error(
      "[portal] /api/gtm/accounts: unrecoverable error:",
      safeErrorName(e),
    );
    return sendJson(res, 500, {
...
// ── Shared helpers (inlined; keep this section in sync across GTM routes) ─
